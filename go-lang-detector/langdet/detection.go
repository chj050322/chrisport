...
func GetDistance(mapA, mapB map[string]int, maxDist int) int {
	...
	for key, rankA := range mapA {
		...
		if rankB, ok := mapB[key]; ok {
			...
			diff = rankB - rankA
			//fmt.Println("key", key, "rank in text", rankA, "rank in mem", rankB, "dis", diff)
